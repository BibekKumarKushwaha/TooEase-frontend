...
const Dashboard = () => {
  ...
  const fetchProducts = (page, searchQuery, sortOrder) => {
    // Sanitize the searchQuery before sending it to the backend
    const sanitizedSearchQuery = DOMPurify.sanitize(searchQuery);

    productPagination(page, limit, sanitizedSearchQuery, sortOrder)
      .then((res) => {
        setProducts(res.data.products); 
      })
      .catch((error) => {
  ...
  const handleSearch = () => {
    // Sanitize the searchQuery before setting it
    const sanitizedSearch = DOMPurify.sanitize(searchQuery);
    setSearchQuery(sanitizedSearch);
    setPage(1);
    fetchProducts(1, sanitizedSearch, sortOrder);
  };
    ...
    <div className="dashboard-container">
      ...
      <div className="search-container mt-4">
        <input
          ...
          placeholder="Search products..."
          value={searchQuery}
          onChange={(e) => setSearchQuery(DOMPurify.sanitize(e.target.value))} // Sanitize input on change
          className="form-control"
        />
        <button onClick={handleSearch} className="btn btn-primary mt-2">Search</button>
      </div>
