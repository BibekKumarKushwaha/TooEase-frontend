...
//     publickKey: "test_public_key_0e1cf205988d4124b151e7a0288cefa4",
...
const Cart = () => {
  ...
  useEffect(() => {
    const totalValue = cartItems.reduce((acc, cart) => acc + cart.total, 0);
    setSubTotal(totalValue);
    setTotal(totalValue + deliveryCharge);
  }, [cartItems, deliveryCharge, change]);

  // --- KHALTI CONFIG & PAYMENT HANDLER (adjusted for paisa) ---
  const khaltiConfig = {
    publicKey: "test_public_key_0e1cf205988d4124b151e7a0288cefa4", // Make sure key is spelled "publicKey" correctly
    productIdentity: "1234567890",
    productName: "Cart Items",
    productUrl: "https://localhost:3000/cart",
    paymentPreference: [
    ...
    eventHandler: {
      onSuccess(payload) {
        ...
        // After successful payment, handle order creation
        handlePayment();
        message.success("Payment successful");
      },
      onError(error) {
  ...
  const handleKhaltiPayment = () => {
    ...
    const checkout = new KhaltiCheckout(khaltiConfig);
    // IMPORTANT: Khalti requires amount in paisa (so multiply by 100)
